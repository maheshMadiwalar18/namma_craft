  ...
  return (
    <div className="bg-cream min-h-screen pb-20 selection:bg-accent/20">
      ...
      <section className="relative py-24 bg-primary text-cream overflow-hidden">
        ...
        <div className="container-custom relative z-10 text-center">
          ...
          >
            ...
            <h1 className="text-3xl md:text-5xl font-display font-bold text-white mb-6">Acquire Timeless Art</h1>
            <p className="text-cream/70 max-w-2xl mx-auto text-base font-light">
      ...
      <section className="container-custom -mt-10 relative z-20">
        <div className="bg-white/80 backdrop-blur-xl p-3 rounded-2xl border border-white/40 shadow-premium flex flex-col md:flex-row gap-3 items-center">
          <div className="relative flex-1 w-full">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-text-soft w-4 h-4" />
            <input 
              type="text" 
              placeholder="Search rare crafts..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2 text-sm rounded-xl bg-cream/30 border border-highlight/10 focus:border-accent outline-none transition-all"
            />
          </div>
          <div className="flex gap-2 w-full md:w-auto">
            <button className="flex-1 md:flex-none btn-secondary !py-2 !px-4 text-sm flex items-center gap-2">
              <Filter className="w-3.5 h-3.5" /> Filters
            </button>
            <button className="flex-1 md:flex-none btn-primary !py-2 !px-4 text-sm">
              Ending Soon
            </button>
          </div>
        </div>
      ...
      <section className="container-custom py-20">
        <div className="grid grid-cols-1 sm:grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {auctions.map((auction, idx) => (
            <motion.div
              ...
              className="group cursor-pointer bg-white rounded-2xl overflow-hidden border border-highlight/10 shadow-sm hover:shadow-premium transition-all duration-500"
            >
              <div className="relative aspect-[16/10] overflow-hidden">
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  ...
                  <h3 className="text-lg font-display font-bold text-white leading-tight group-hover:text-accent transition-colors">{auction.name}</h3>
                </div>
              </div>
              ...
              <div className="p-5">
                <div className="flex justify-between items-end mb-4">
                  <div>
                    ...
                    <p className="text-xl font-display font-bold text-primary">₹{auction.currentBid.toLocaleString()}</p>
                  </div>
                ...
                <button className="w-full btn-primary !py-2 text-sm flex items-center justify-center gap-2 group/btn">
      ...
      <section className="container-custom pb-20">
        <div className="bg-accent/5 rounded-[48px] p-12 md:p-20 text-center relative overflow-hidden border border-accent/10">
          ...
          <h2 className="text-2xl font-display font-bold text-primary mb-4">Have a rare masterpiece?</h2>
          ...
          <button className="btn-accent px-10 py-3 text-base">Apply as Artisan</button>
        </div>
      </section>
    </div>
  );
